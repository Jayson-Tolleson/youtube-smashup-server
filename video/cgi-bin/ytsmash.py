...
ffmpeg = ('''cd /var/video/'''+pid+'''/videos2/ && sudo ffmpeg -f concat -safe 0 -i vidslist.txt -vf mpdecimate,setpts=N/FRAME_RATE/TB -y output.mp4 && sudo cp output.mp4 /var/video/movies/'''+str(searchterms).replace(' ',',').replace('/',',')+str(number)+'''.mp4''')
