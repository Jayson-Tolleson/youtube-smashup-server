...
ffmpeg = ('''cd /var/video/'''+pid+'''/videos2/ && sudo ffmpeg -f concat -safe 0 -i vidslist.txt -c:v libx264 -vf mpdecimate,setpts=N/FRAME_RATE/TB,"pad=ceil(iw/2)*2:ceil(ih/2)*2" -shortest -cpu-used -5 -deadline realtime -y output.mp4 && sudo cp output.mp4 /var/video/movies/'''+str(searchterms).replace(' ',',').replace('/',',')+str(number)+'''.mp4''')    
